...
patch(PosStore.prototype, {
    ...
    async addLineToOrder(vals, order, opts = {}, configure = true) {
        ...
        // console.log("are you the one calling >>>.")
        ...
        this.productProduct = await this.data.orm.searchRead("product.product", [['id','=',product.id]], ['linnworks_location_id','name','linnworks_item_id']);
        
        let result = 0
        // console.log("Inner methoid Vals >>...",vals)
        if(this.productProduct[0].linnworks_item_id){
            // this.linnworks_location_id = this.productProduct[0].linnworks_location_id
            this.linnworks_item_id = this.productProduct[0].linnworks_item_id
            let data = {
                // 'linnworks_location_id' : this.linnworks_location_id,
                'linnworks_item_id' : this.linnworks_item_id,
            }
            result = await rpc('/linnworks/get_stock_level_batch', data)
        }
        // console.log("result 1 >>>>",result)
        
        if ( result != 0 && values['qty'] > result) {
            this.dialog.add(AlertDialog, {
                title: _t(`Max quantity exceed!!`),
                body: _t(`you can only buy ${result} qty of ${product.name}`),
            });
            return
        }
        else if (result == 0) {
            this.dialog.add(AlertDialog, {
                ...
                body: _t(`There is no stock available of ${product.name}`),
            });
            return
        }
        

        values['available_linn_qty'] = result
        ...
        if(to_merge_orderline){
            line.delete()
            if ( result != 0 && (values['qty'] + to_merge_orderline['qty'] || 0) > result) {
                this.dialog.add(AlertDialog, {
                    title: _t(`Max quantity exceed!!`),
                    body: _t(`you can only buy ${result} qty of ${product.name}`),
                });
        ...
        //### we need to set available qty here 
